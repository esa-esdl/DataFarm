...
function genDataset{T<:ProcessMean,U<:Noise,V<:Disturbance,W<:Noise}(mt::Vector{T},nt::Vector{U},dt::Vector{V},dataNoise::Vector{W},Ncomp,Nvar,Nlon,Nlat,Ntime,k)
    ...
    for i = 1:Ncomp
        ...
        x,d = genCube(mt[i<=length(mt) ? i : 1],nt[i<=length(nt) ? i : 1],dt[i<=length(dt) ? i : 1],Nlon,Nlat,Ntime,k)
        acomp[i,:,:,:]=x
        dcomp[i,:,:,:]=d
    ...
    for i=1:Nvar
        ...
        w = w./sum(w)
        varnoise = genNoise(datanoise[i<=length(datanoise) ? i : 1],Nlon,Nlat,Ntime)
